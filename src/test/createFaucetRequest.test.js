...
describe('createFaucetRequest', () => {
  describe('handler()', () => {
    it('should return http status 400 when no valid body supplied', () => {
        ...
        const expectedBody = JSON.stringify({message:"Body cannot be empty"});
    ...
    it('should return 400 when the address is burn address', () => {
        ...
        const expectedBody = JSON.stringify({message:"No burning"});
        faucet.handler(invalidEvent, null, (err, response) => {  
            assert.equal(response.statusCode, 400);            
            assert.equal(response.body, expectedBody);
        });
    ...
    it('should return 400 when the address invalid', () => {
        ...
        const body = JSON.stringify({address: invalidAddress});
        const invalidEvent = {
            resource: "/",
            path: "/request",
            httpMethod: "POST",
        ...
        const expectedBody = JSON.stringify({message:`Address ${invalidAddress} is not valid`});
        faucet.handler(invalidEvent, null, (err, response) => {            
            assert.equal(response.statusCode, 400);            
            assert.equal(response.body, expectedBody);
        });
